...
router.get("/user/:id", authVerify, async (req, res) => {
  try {
    const details = await BookingDetails.find({ user: req.params.id })
      .populate("user")
      .populate("film")
      .populate("theater")
      .populate("show");
...
router.get("/film/:id", authVerify, async (req, res) => {
  try {
    const details = await BookingDetails.find({ film: req.params.id })
      .populate("user")
      .populate("film")
      .populate("theater")
      .populate("show");
...
router.get("/theater/:id", authVerify, async (req, res) => {
  try {
    const details = await BookingDetails.find({ theater: req.params.id })
      .populate("user")
      .populate("film")
      .populate("theater")
      .populate("show");
...
router.post("/create-booking", authVerify, async (req, res) => {
  try {
...
router.put("/:id", authVerify, async (req, res) => {
  try {
    const details = await BookingDetails.updateOne(
      { _id: req.params.id },
      req.body
    );
    res.send({
